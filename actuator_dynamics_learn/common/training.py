...
def train(training_dataloader, validation_dataloader, device, optimizer, network, writer, batch_size=64, epochs=8,
    ...
    for epoch in range(epochs):
        ...
        # Train and Validate
        for training_iteratable, validation_iterable in zip(training_dataloader,
                                                            itertools.cycle(validation_dataloader)):
            training_batch, training_targets = training_iteratable[0].to(device), training_iteratable[1].to(device)
            network.train()
